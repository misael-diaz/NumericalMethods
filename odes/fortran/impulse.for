!
!   Applied Numerical Analysis                              August 27, 2021
...
!                       y' + k * y = b * u(t),
!
!   where k is the rate, b is the forcing constant, and u(t) = 1 is the
!   unit-step function for t > 0. The response to a unit-impulse is
!   simply the first-derivative y'(t) of the unit-step response.
!
!
...
module impulse_odefuns
    ...
    contains
        ...
        function fstep(t) result(y)
            ! Synopsis: Analytic expression for the step response y(t).
            real(kind = real64) :: y
            real(kind = real64), intent(in) :: t
        ...
        function fimpulse(t) result(y)
            ! Synopsis: Analytic expression for the impulse response y(t).
            real(kind = real64) :: y
            real(kind = real64), intent(in) :: t
            real(kind = real64), parameter :: k  = RATE
            real(kind = real64), parameter :: b  = FEXT
        ...
        subroutine odesol_Write (odesol, filename)
            ...
            real(kind = real64) :: err_step, err_impulse, params(2)
...
program tests
    ...
    use impulse_odefuns, only: odefun
    use impulse_odefuns, only: write => odesol_Write
    implicit none
    ...
    integer(kind = int64), parameter :: n = 511                 ! num steps
    ...
    character(*), parameter :: BaseFilename = "output/impulse/Euler.dat"
