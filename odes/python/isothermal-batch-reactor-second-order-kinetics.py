...
Synopsis:
Solves for the transient response of a reactant depleted by a chemical
reaction having second-order kinetics in a batch reactor at constant
...
[3] HS Fogler, Elements of Chemical Reaction Engineering, 6th edition
"""
...
yi, k = (1.0, 1.0)                              # initial value and rate
odefun = lambda t, y: ( -k * y**2 )             # ODE RHS, f(t, y)
kinetics = lambda t: ( 1.0 / (1.0 + k * t) )    # analytic solution, y(t)


n = 255                         # number of integration time-steps
ti, tf = (0.0, 2.0e1)           # initial time ti and final time tf
tspan  = (ti, tf)               # time span tuple
odesol = np.empty([2, n+1, 2])  # preallocates array for speed


""" solves the ODE with Euler's and Runge-Kutta Methods """
...
ax.plot(t, kinetics(t), color="black", linewidth=2.0,
...
ax.set_ylabel("dynamic response, reactant concentration, y(t)")
ax.set_title("Second Order Reaction Kinetics")

# exports figure as a Portable Network Graphic PNG with 300 DPI resolution
fig.savefig("output/cheme/kinetics/second-order-kinetics.png", dpi=300)
