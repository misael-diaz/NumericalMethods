/*
 * Transient Heat Conduction                                		May 28, 2023
 *
 * source: test.c
 * author: @misael-diaz
 *
 ...
 *
 * The objective is to write a solver that vectorizes the cpu expensive loops.
 * The *restrict* keyword is used to promise the compiler that there is no
 * array aliasing. I have added inline comments to indicate which loops have been
 * vectorized. For a full list, see the optimization report produced by GCC opts.log.
 *
 *
...
void rhs (size_t size, double* restrict g, const double* restrict b)
{
  size_t N = (size - 1);
...
void tridiag (size_t size, double* restrict g, const double* restrict g0)
{
  size_t N = (size - 1);
  for (size_t i = 1; i != N; ++i)
  {
...
void __attribute__ ((noinline)) scale (size_t size, double* g)
{
  size_t N = (size - 1);
...
void solver (workspace_t* workspace)
{
  ...
  for (size_t i = 0; i != iters; ++i)
  {
    ...
    rhs(size, g, b);				// vectorized by gcc
    tridiag(size, g, g0);			// vectorized by gcc
    scale(size, g);				// vectorized by gcc
